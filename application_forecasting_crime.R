...
crime_data = read.csv("datasets/crime_data.csv", header = TRUE)
crime_data$Date <- as.Date(crime_data$Date, format = "%m/%d/%Y")
Y = as.matrix(crime_data[, 2:ncol(crime_data)])
...
for(i in 1:n){
  ...
  dfc_mse = dfc_results[1:H, "MAE", alpha_level]
  var_mse = var_results[1:H, "MAE", alpha_level]
  dlm_mse = dlm_results[1:H, "MAE", alpha_level]
  nbinom_mse = nbinom_results[1:H, "MAE", alpha_level]
  
  L = min(dfc_mse, var_mse, dlm_mse, nbinom_mse)
  U = max(dfc_mse, var_mse, dlm_mse, nbinom_mse)
  
  plot(1:H, dfc_mse, type = "l", col = dfc_col, ylim = c(0.95*L, 1.05*U), xaxt = "n",
       xlab = "forecast horizon", ylab = "MAFE", main = dgp)
  lines(1:H, var_mse, col = var_col)
  lines(1:H, dlm_mse, col = dlm_col)
  #lines(1:H, nbinom_mse, col = nbinom_col)
  
  points(1:H, dfc_mse, col = dfc_col, pch = 19)
  points(1:H, var_mse, col = var_col, pch = 19)
  points(1:H, dlm_mse, col = dlm_col, pch = 19)
  #points(1:H, nbinom_mse, col = nbinom_col, pch = 19)
  ...
  nbinom_int = nbinom_results[1:H, "INT-SIZE", alpha_level]
